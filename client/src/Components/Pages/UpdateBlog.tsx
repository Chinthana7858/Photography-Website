...
function UpdateBlog() {
  ...
  const publish = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (userConfirmed) {
      ...
      try {
        const response = await axios.put(`http://localhost:8080/api/v1/blog/${blogId}/updateIsPublished`, formData, {
          headers: {
  ...
  const deleteBlog = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (userConfirmed) {
      ...
      try {
        const response = await axios.delete(`http://localhost:8080/api/v1/blog/${blogId}`, {
        });
  ...
  const deleteSubBlog = async (event: React.FormEvent<HTMLFormElement>, subBlogId: string) => {
    ...
    if (userConfirmed) {
      ...
      try {
        const response = await axios.delete(`http://localhost:8080/api/v1/blogSubTopic/${subBlogId}`, {
        });
        alert("Deleted successfully!");
        console.log("Deleted successfully!", response.data);
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/api/v1/blogSubTopic/byBlog/${blogId}`);
