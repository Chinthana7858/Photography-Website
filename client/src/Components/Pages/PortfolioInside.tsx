...
function PortfolioInside() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/api/v1/portfolio/${portfolioId}`);
  ...
  const handleDeleteSection= async () => {
    ...
    axios
      .delete(`http://localhost:8080/api/v1/portfolio/${portfolio?.id}`)
      .then((response) => {
  ...
  const handleDeleteImg = (id: string) => {
    ...
    axios
      .delete(`http://localhost:8080/api/v1/images/category/${id}`)
      .then((response) => {
        console.log('Response:', response.data);
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8080/api/v1/images/category/${portfolioId}`)
      .then((response) => {
    ...
    <div>
      ...
      <div className="md:pt-[10%] sm:pt-[20%] xs:pt-[25%] pl-[3%]">
        ...
        <div className="sm:flex xs:flex-col">
          ...
          <div className="px-6 py-3 ">{portfolio?.about}</div>
          <div className="text-center py-[2vh]">-Chinthana Prabhashitha-</div>
          </div>
        </div>

        <div className="z-0 pt-2 columns-2 md:columns-3 lg:columns-4">
        {images.map((image) => (
